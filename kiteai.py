...
def load_wallet_from_env():
    ...
    # If not found in .env, try to load from tokens.txt
    if not wallet_address:
        try:
            ...
            print(f"{Fore.RED}Error loading wallet address from tokens.txt: {e}{Style.RESET_ALL}")
...
╔══════════════════════════════════════════════╗
║               KITE AI AUTOMATE               ║
║    Github: @im-hanzou & @arcxteam (0xgrey)   ║
╚══════════════════════════════════════════════╝
    """
    print(Fore.CYAN + print_banner + Style.RESET_ALL)
    
    wallet_address = load_wallet_from_env()
    
    if not wallet_address:
        print(f"{Fore.RED}No wallet address found. Please add your wallet address to .env file as WALLET_ADDRESS=your_address or to tokens.txt{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}You can register at: {Fore.GREEN}https://testnet.gokite.ai?r=cmuST6sG{Style.RESET_ALL}")
        exit(1)
    
    print(f"{Fore.GREEN}Loaded wallet address: {wallet_address}{Style.RESET_ALL}")
